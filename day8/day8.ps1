...
foreach( $line in Get-Content -Path $Path )
{
    ...
    # get length of raw code
    $codeLength += $line.length
    Write-Verbose -Message "`tCode length: $codeLength"

    # determine length of string based on the escape sequences
    $stringLength += ( $line -replace '^"','' -replace '"$' -replace '\\\\','\' -replace '\\"','"' -replace '\\x[0-9A-Fa-f]{2}','x' ).length
    Write-Verbose -Message "`tString length: $stringLength"

    # re-encode the code line and count the number of characters
    $encodedLength += ( '"' + ( $line -replace '\\','\\' -replace '"','\"' ) + '"' ).length
} # end foreach line

Write-Host "Total code length: $codeLength"
Write-Host "Total string length: $stringLength"
Write-Host "Code to string length difference: $($codeLength - $stringLength)"

Write-Host "Total encoded string length: $encodedLength"
Write-Host "Encoded to Code length difference: $($encodedLength - $codeLength)"